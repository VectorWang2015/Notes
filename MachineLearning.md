...
在某一阈值下,模型将大于该阈值的视作正样本,小于该阈值的视作负样本,此时返回结果对应一组Precsion和Recall.通过将阈值从高到低移动,获得一系列的PR值,以召回率为横坐标,精确率为纵坐标,可得PR曲线.一个P-R曲线如下图,横坐标为0时阈值最大.  
